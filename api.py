...
def api_register(request):
    ...
    if error:
        logger.info("Register failed", extra={"route": "register", "username": username, "reason": error})
    ...
    logger.info("User registered successfully", extra={"route": "register", "username": username})
...
def api_msgs(request):
    ...
    results = [
        {
            "content": msg.text,
            "pub_date": format_api_datetime(msg.pub_date),
            "user": usr.username,
        }
...
def api_user_msgs_get(request):
    ...
    messages_query = (
        request.db.query(Message, User)
        .join(User, Message.author_id == User.user_id)
        .filter(Message.flagged == 0, User.user_id == user_id)
        .order_by(Message.pub_date.desc())
        .limit(no)
        .all()
    )
...
def api_metrics(request):
    ...
    return {"total_users": total_users, "average_followers": round(avg_followers, 2)}
    ...
    if "follow" in data:
        ...
        if whom_id is None:
            logger.warning(
                ...
                extra={"route": "api_follows_post", "username": username, "action": action, "target": whom_username},
            )
            return Response(json={"status": 404, "error_msg": "User not found"}, status=404)
        ...
        if whom_id is None:
            logger.info(
                "Target not found for unfollow (ignored)",
                extra={"route": "api_follows_post", "username": username, "action": action, "target": whom_username},
            )
            return Response(status=204)
