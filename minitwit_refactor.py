...
def gravatar_url(email, size=80):
    ...
    email_hash = md5(email_bytes, usedforsecurity=False).hexdigest() 
...
def user_timeline(request):
    ...
    if profile_user_obj is None:
        return HTTPNotFound()
    profile_user = {
        "user_id": profile_user_obj.user_id,
        "username": profile_user_obj.username,
        "email": profile_user_obj.email,
    }
    ...
    if request.user:
        check = (
            ...
            .filter(
                Follower.who_id == request.session["user_id"],
    ...
    messages_query = (
        request.db.query(Message, User)
        .join(User, Message.author_id == User.user_id)
        .filter(Message.flagged == 0, User.user_id == profile_user["user_id"])
        .order_by(Message.pub_date.desc())
        .limit(PER_PAGE)
        .all()
    )
